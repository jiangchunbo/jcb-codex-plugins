...
Before sending a contract, check the two easy-to-miss shapes:

- `ready` is an expectation, not a target: use `"ready":{"target":{"text":"Ready"},"state":"visible"}`, never `"ready":{"text":"Ready","state":"visible"}`.
- `routes` and `blockResourceTypes` belong to this contract only. Browser, cookies, and local storage can persist, but network rules are removed when `run` returns.
...
Route rules accept `url` glob, optional `method`, partial-JSON `requestBody` or string `requestBodyIncludes`, and one of `json`, `body`, or `abort`. They are installed before this contract's navigation and removed before the result returns, so repeat required mocks in every later contract that navigates or reloads. In a URL glob, `?` matches exactly one character; it does not make a query string optional. For example, `**/toc/orders?*` misses `/toc/orders`, while `**/toc/orders*` covers that URL with or without a query string. Set `cors:true` to add credential-compatible response headers and fulfill matching OPTIONS requests surfaced by Playwright. Claim that a preflight occurred only when `captureRouteCalls:true` returns `action:"cors-preflight"`. Optional `status`, `headers`, and `contentType` customize fulfillment.
