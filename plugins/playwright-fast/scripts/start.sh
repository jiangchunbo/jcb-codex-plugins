...
if ! runtime_line=$(node "$script_dir/resolve-runtime.js" --lines); then
  exit 1
fi
...
exec env \
  ...
  PLAYWRIGHT_FAST_INSTALL_COMMAND="bash '$plugin_dir/scripts/install-runtime.sh'" \
  node "$plugin_dir/scripts/server.js"
